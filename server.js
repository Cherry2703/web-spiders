...
// app.get('/tasks/', middleWare, async (request, response) => {
...
// app.get('/tasks/:taskId/', middleWare, async (request, response) => {
//     try {
...
// Middleware to parse JSON and enable CORS
app.use(express.json());
app.use(cors()); // Add CORS options if needed
...
// Basic route to check server status
app.get("/", (request, response) => {
    response.send('Tasks backend testing is working... go for different routes');
});
...
// Route to get all tasks for the logged-in user (protected by middleware)
app.get('/tasks/', middleWare, async (request, response) => {
    const userQuery = `SELECT * FROM users WHERE username = '${request.username}';`;
    const user = await db.get(userQuery);

    if (user) {
        const tasks = await getAllTasksForUser(user.user_id);
...
// Route to create a new task (protected by middleware)
app.post('/tasks/', middleWare, async (request, response) => {
    try {
        const userQuery = `SELECT * FROM users WHERE username = '${request.username}';`;
        const user = await db.get(userQuery);
...
// Route to update a task by ID (protected by middleware)
app.put('/tasks/:task_id', middleWare, async (request, response) => {
    try {
        ...
        if (user) {
            ...
            if (task && task.user_id === user.user_id) {
                ...
                response.status(404).send({ message: "Task not found or you don't have permission to update it." });
            }
        } else {
            response.status(404).send({ message: "User not found." });
        }
...
// Route to delete a task by ID (protected by middleware)
app.delete('/tasks/:task_id', middleWare, async (request, response) => {
    try {
        const { task_id } = request.params;
        ...
        if (user) {
            ...
            if (task && task.user_id === user.user_id) {
                ...
                response.status(404).send({ message: "Task not found or you don't have permission to delete it." });
            }
        } else {
            response.status(404).send({ message: "User not found." });
        }
